...
const CombinedTotalSexAge = ({ selectedPathology, selectedMun }) => {
    ...
    const fetchData = async () => {
      try {
        const promises = selectedMun.map(mun =>
          fetchPrevalenceDataGroup(['municipality', 'patology', 'sex', 'mun_name', 'age_group','measure_normalized'], mun.id.toString(), selectedPathology)
        );
    ...
    if (selectedOption === 'sex') {
      ...
      const groupedData = prevalenceData.reduce((acc, entry) => {
        ...
        if (existingIndex === -1) {
          acc.push({
            municipality: entry.municipality,
            total: entry.measure_normalized,
            mun_name: entry.mun_name,
          });
        } else {
